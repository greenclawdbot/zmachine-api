...
(function (OperandType) {
    ...
    OperandType[OperandType["Large"] = 0] = "Large";
    //  $$01    Small constant (0 to 255)      1 byte
    OperandType[OperandType["Small"] = 1] = "Small";
    //  $$10    Variable                       1 byte
    OperandType[OperandType["Variable"] = 2] = "Variable";
    //  $$11    Omitted altogether             0 bytes
    OperandType[OperandType["Omitted"] = 3] = "Omitted";
})(OperandType || (OperandType = {}));
var InstructionForm;
(function (InstructionForm) {
    InstructionForm[InstructionForm["Long"] = 0] = "Long";
    InstructionForm[InstructionForm["Short"] = 1] = "Short";
    InstructionForm[InstructionForm["Variable"] = 2] = "Variable";
    InstructionForm[InstructionForm["Extended"] = 3] = "Extended";
...
(function (KnownGlobals) {
    KnownGlobals[KnownGlobals["Location"] = 0] = "Location";
    // for score games:
    KnownGlobals[KnownGlobals["Score"] = 1] = "Score";
    KnownGlobals[KnownGlobals["NumTurns"] = 2] = "NumTurns";
    // for time games:
    KnownGlobals[KnownGlobals["Hours"] = 1] = "Hours";
    KnownGlobals[KnownGlobals["Minutes"] = 2] = "Minutes";
})(KnownGlobals || (KnownGlobals = {}));
var HeaderLocation;
(function (HeaderLocation) {
    HeaderLocation[HeaderLocation["Version"] = 0] = "Version";
    HeaderLocation[HeaderLocation["Flags1"] = 1] = "Flags1";
    HeaderLocation[HeaderLocation["HighMemBase"] = 4] = "HighMemBase";
    HeaderLocation[HeaderLocation["InitialPC"] = 6] = "InitialPC";
    HeaderLocation[HeaderLocation["Dictionary"] = 8] = "Dictionary";
    HeaderLocation[HeaderLocation["ObjectTable"] = 10] = "ObjectTable";
    HeaderLocation[HeaderLocation["GlobalVariables"] = 12] = "GlobalVariables";
    HeaderLocation[HeaderLocation["StaticMemBase"] = 14] = "StaticMemBase";
    HeaderLocation[HeaderLocation["Flags2"] = 16] = "Flags2";
    HeaderLocation[HeaderLocation["AbbreviationsTable"] = 24] = "AbbreviationsTable";
    HeaderLocation[HeaderLocation["InterpreterNumber"] = 30] = "InterpreterNumber";
    HeaderLocation[HeaderLocation["InterpreterVersion"] = 31] = "InterpreterVersion";
    HeaderLocation[HeaderLocation["ScreenHeightInLines"] = 32] = "ScreenHeightInLines";
    HeaderLocation[HeaderLocation["ScreenWidthInChars"] = 33] = "ScreenWidthInChars";
    HeaderLocation[HeaderLocation["ScreenWidthInUnits"] = 34] = "ScreenWidthInUnits";
    HeaderLocation[HeaderLocation["ScreenHeightInUnits"] = 36] = "ScreenHeightInUnits";
    // more version 5/6 stuff after this
    HeaderLocation[HeaderLocation["RoutinesOffset"] = 40] = "RoutinesOffset";
    HeaderLocation[HeaderLocation["StaticStringsOffset"] = 42] = "StaticStringsOffset";
})(HeaderLocation || (HeaderLocation = {}));
export default class Game {
    constructor(story_buffer, log, screen, storage) {
    ...
    static fromSnapshot(snapshotBuffer, log, screen, storage) {
        const { mem, stack, callstack, pc } = Game.readSnapshotFromBuffer(snapshotBuffer);
    ...
    snapshotToBuffer() {
        console.log(`at snapshot time, mem is length ${this._mem.length}, and pc = ${this.pc}`);
    ...
    static readSnapshotFromBuffer(b) {
        ...
        console.log(`at snapshot time, mem is length ${memToUse.length}, and pc = ${pcToUse}`);
        return {
            mem: memToUse,
            stack: stackToUse,
    ...
    continueAfterKeyPress(input_state, _key) {
    ...
    executeInstruction() {
        ...
        try {
            ...
            console.error(e);
            this._log.error(`error at pc=${hex(op_pc)}, opcode=${hex(opcode)}: ${e.toString()}`);
            throw e;
        }
    ...
    pushStack(v) {
        ...
        this._stack.push(v);
        this._log.debug(`     after pushStack(${hex(v)}): ${this._stack.map((el) => hex(el))}`);
    }
    popStack() {
    ...
    storeVariable(v, value, replaceTop = false) {
        ...
        if (v < 16) {
            ...
            if (v > cur_frame.locals.length) {
                throw new Error(`local ${v} out of range.  there are ${cur_frame.locals.length} locals`);
            }
    ...
    encodeToken(text, padding = 0x05) {
        ...
        for (let i = 0; i < resolution; i++) {
            zwords.push((zchars[3 * i + 0] << 10) | (zchars[3 * i + 1] << 5) | zchars[3 * i + 2]);
        }
    ...
    tokeniseLine(textBuffer, parseBuffer, dict, flag) {
        ...
        do {
            ...
            if (sep_count == 0 && c != 32 && c != 0) {
                ...
                this.tokeniseText(textBuffer, addr1 - addr2, addr2 - textBuffer, parseBuffer, dict, flag);
                addr2 = 0;
            }
            if (sep_count != 0) {
                this.tokeniseText(textBuffer, 1, addr1 - textBuffer, parseBuffer, dict, flag);
            }
        } while (c != 0);
    }
    tokenise(inputtext, parsebuffer) {
        ...
        this._log.debug(`sep_zscii = ${sep_zscii.map((ch) => String.fromCharCode(ch))}`);
    ...
    updateStatusBar() {
        ...
        const isScoreGame = this._version < 3 || (this.getByte(HeaderLocation.Flags1) & 0x02) == 0;
        const location = this.getWord(this._global_vars + 2 * KnownGlobals.Location);
        ...
        if (isScoreGame) {
            ...
            const minutes = this.getWord(this._global_vars + 2 * KnownGlobals.Minutes);
            rhs = `Time: ${hours}:${minutes} `;
        }
        this._screen.updateStatusBar(lhs, rhs);
    }
}
//# sourceMappingURL=ebozz.js.map