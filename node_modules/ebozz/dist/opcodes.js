...
function je(s, a, b, c, d) {
    ...
    s._log.debug(`${hex(s.op_pc)} je ${hex(a)} ${hex(b)} ${hex(c)} ${hex(d)} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
    const cond = a === b || (c !== undefined && a === c) || (d !== undefined && a === d);
    s.doBranch(cond, condfalse, offset);
}
function jl(s, a, b) {
    const [offset, condfalse] = s.readBranchOffset();
    s._log.debug(`${hex(s.op_pc)} jl ${hex(a)} ${hex(b)} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
    s.doBranch(toI16(a) < toI16(b), condfalse, offset);
}
function jg(s, a, b) {
    const [offset, condfalse] = s.readBranchOffset();
    s._log.debug(`${hex(s.op_pc)} jg ${hex(a)} ${hex(b)} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
    s.doBranch(toI16(a) > toI16(b), condfalse, offset);
}
function jin(s, obj1, obj2) {
    const [offset, condfalse] = s.readBranchOffset();
    s._log.debug(`${hex(s.op_pc)} jin ${hex(obj1)} ${hex(obj2)} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
...
function jz(s, a) {
    const [offset, condfalse] = s.readBranchOffset();
    s._log.debug(`${hex(s.op_pc)} jz ${hex(a)} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
    /*
    if (a === 2) {
...
function test(s, bitmap, flags) {
    const [offset, condfalse] = s.readBranchOffset();
    s._log.debug(`${hex(s.op_pc)} test ${hex(bitmap)} ${hex(flags)} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
...
function test_attr(s, obj, attribute) {
    const [offset, condfalse] = s.readBranchOffset();
    s._log.debug(`${hex(s.op_pc)} test_attr ${hex(obj)} ${hex(attribute)} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
...
function get_prop(s, obj, property) {
    ...
    s._log.debug(`${hex(s.op_pc)} get_prop ${hex(obj)} ${hex(property)} -> (${hex(resultVar)})`);
    const o = s.getObject(obj);
    if (o === null) {
...
function get_prop_addr(s, obj, property) {
    const resultVar = s.readByte();
    s._log.debug(`${hex(s.op_pc)} get_prop_addr ${hex(obj)} ${hex(property)} -> (${hex(resultVar)})`);
    const o = s.getObject(obj);
    if (o === null) {
...
function get_next_prop(s, obj, property) {
    const resultVar = s.readByte();
    s._log.debug(`${hex(s.op_pc)} get_next_prop ${hex(obj)} ${hex(property)} -> (${hex(resultVar)})`);
    const o = s.getObject(obj);
    if (o === null) {
...
function get_sibling(s, obj) {
    const resultVar = s.readByte();
    const [offset, condfalse] = s.readBranchOffset();
    s._log.debug(`${hex(s.op_pc)} get_sibling ${hex(obj)} -> (${hex(resultVar)}) ?[${!condfalse}] ${hex(offset)}`);
    const o = s.getObject(obj);
    let sibling = null;
    if (o) {
        sibling = o.sibling;
...
function get_child(s, obj) {
    const resultVar = s.readByte();
    const [offset, condfalse] = s.readBranchOffset();
    s._log.debug(`${hex(s.op_pc)} get_child ${hex(obj)} -> (${hex(resultVar)}) ?[${!condfalse}] ${hex(offset)}`);
    const o = s.getObject(obj);
    if (o === null) {
...
function get_prop_len(s, propDataAddr) {
    const resultVar = s.readByte();
    s._log.debug(`${hex(s.op_pc)} get_prop_len ${hex(propDataAddr)} -> (${hex(resultVar)})`);
...
function dec_chk(s, variable, value) {
    const [offset, condfalse] = s.readBranchOffset();
    s._log.debug(`${hex(s.op_pc)} dec_chk ${hex(variable)} ${value} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
...
function inc_chk(s, variable, value) {
    const [offset, condfalse] = s.readBranchOffset();
    s._log.debug(`${hex(s.op_pc)} inc_chk ${hex(variable)} ${value} -> [${!condfalse}] ${hex(s.pc + offset - 2)}`);
    const new_val = toI16(s.loadVariable(variable, true)) + 1;
    s.storeVariable(variable, toU16(new_val), true);
    s._log.debug(`     ${new_val} ?> ${value}`);
    s.doBranch(new_val > toI16(value), condfalse, offset);
...
function inc(s, variable) {
    s.storeVariable(variable, toU16(toI16(s.loadVariable(variable, true)) + 1), true);
}
...
function storew(s, array, word_index, value) {
    s._log.debug(`${hex(s.op_pc)} storew ${hex(array)} ${hex(word_index)} ${hex(value)}`);
    s.setWord((array + 2 * word_index) & 0xffff, value);
}
function storeb(s, array, byte_index, value) {
    s._log.debug(`${hex(s.op_pc)} storeb ${hex(array)} ${hex(byte_index)} ${hex(value)}`);
...
function loadw(s, array, word_index) {
    const resultVar = s.readByte();
    s._log.debug(`${hex(s.op_pc)} loadw ${hex(array)} ${hex(word_index)} -> (${hex(resultVar)})`);
    s.storeVariable(resultVar, s.getWord((array + 2 * word_index) & 0xffff));
}
function loadb(s, array, byte_index) {
    const resultVar = s.readByte();
    s._log.debug(`${hex(s.op_pc)} loadb ${hex(array)} ${hex(byte_index)} -> (${hex(resultVar)})`);
...
function call_1s(s, routine) {
    ...
    s._log.debug(`${hex(s.op_pc)} call_1s ${hex(routine)} -> (${hex(resultVar)})`);
...
function call_2s(s, routine, arg1) {
    ...
    routine = s.unpackRoutineAddress(routine);
    s._log.debug(`${hex(s.op_pc)} call_2s ${hex(routine)} ${arg1} -> (${hex(resultVar)})`);
...
function call_vs2(s, routine, ...args) {
    ...
    routine = s.unpackRoutineAddress(routine);
    s._log.debug(`${hex(s.op_pc)} call_vs2 ${hex(routine)} ${args} -> (${hex(resultVar)})`);
...
function call(s, routine, ...args) {
    ...
    routine = s.unpackRoutineAddress(routine);
    s._log.debug(`${hex(s.op_pc)} call ${hex(routine)} ${args} -> (${hex(resultVar)})`);
    s.callRoutine(routine, resultVar, ...args);
}
...
function print_paddr(s, packed_addr) {
    s._screen.print(s, zstringToAscii(s, s.getZString(s.unpackStringAddress(packed_addr)), true));
}
function new_line(s) {
    s._screen.print(s, "\n");
}
...
function output_stream(s, streamNum, table, width) {
...
function input_stream(s, streamNum) {
...
function sread(s, textBuffer, parseBuffer, time, routine) {
    ...
    s._log.debug(`sread max_input=${max_input}, text=${textBuffer}, parse=${parseBuffer}, time=${time}, routine=${routine}`);
    s.updateStatusBar();
